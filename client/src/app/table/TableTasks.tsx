    ...
    const renderUsers = (tasks: Task[]) => {
        const renderedUsers =  tasks.map((task: Task) =>
                <Table.Row key={task.id} className={`${styles.table__row}`}
                    ...
                    <Table.RowHeaderCell className={styles.table__cell}>{task.title}</Table.RowHeaderCell>
                    {task.isDone
                        ? <Table.Cell className={`${styles.table__cell} ${styles.task__status} ${styles.task__done}`}>Done</Table.Cell>
                        : <Table.Cell className={`${styles.table__cell} ${styles.task__status} ${styles.task__processing}`}>In progress</Table.Cell>
                    }
            ...
            <Table.Root variant={'surface'} className={styles.table}>
                <Table.Header>
                    <Table.Row className={`${styles.table__row} ${styles.table__header}`}>
                        <Table.ColumnHeaderCell className={styles.table__cell}>Title</Table.ColumnHeaderCell>
